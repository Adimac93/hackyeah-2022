...
export const actions: Actions = {
    default: async (event) => {
        ...
        if (!offerId || !count) return invalid(400, {});

        let countI = Number.parseInt(count);
        if (countI <= 0) return invalid(400, {});

        let offer = await db.shopOffert.findUnique({ where: { id: offerId } });
        if (!offer) return invalid(400, {});

        if (!UserHelper.spendCoins(user, offer.cost * countI)) return { success: false };
